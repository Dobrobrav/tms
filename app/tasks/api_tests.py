...
class TestTaskAPI:
    ...
    def test__when_task_is_created_then_it_can_be_retrieved(self) -> None:
        ...
        assert get_task_response.json()['task_id'] == created_task_id

    def _create_user(self, username: str) -> int:
        response = APIClient().post('tasks/tasks/users/', data={'name': username})
    ...
    def _create_2_tasks(self, reporter_id: int) -> list[int]:
        ...
        created_task_ids = [
            APIClient().post('tasks/tasks/', data=task_data_1).json()['id'],
